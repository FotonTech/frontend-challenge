  ...
  useEffect(() => {
    ...
    axios.all([
      ...
      .then(axios.spread((res1, res2, res3, res4, res5, res6) => {
        setBanners(banners => [...banners, res1.data.items[0].volumeInfo])
        setBanners(banners => [...banners, res2.data.items[2].volumeInfo])
        setBanners(banners => [...banners, res3.data.items[0].volumeInfo])
        setBanners(banners => [...banners, res4.data.items[0].volumeInfo])
        setBanners(banners => [...banners, res5.data.items[0].volumeInfo])
        setBanners(banners => [...banners, res6.data.items[0].volumeInfo])
      }), error => {
        console.error(error);
      });
    ...
    axios.get(`https://www.googleapis.com/books/v1/volumes?q=${currentlyReadingBookQuery}`)
      .then(response => {
        setCurrentlyReadingBook(response.data.items[0].volumeInfo);
      }, error => {
        console.error(error);
      })
  }, []);
      ...
      <main>
          ...
          >
            {banners.map((banner, index) => (
              <Banner
                ...
                bookName={banner.title !== undefined ? banner.title : "Unknown title"}
                bookAuthor={banner.authors !== undefined ? banner.authors[0] : "Unknown author"}
                pageCount={index === 0 ? 120 : (
                  index === 1 ? 90 : (
                    banner.pageCount !== undefined ?
                      round(banner.pageCount) : 1
                  )
                )}
                bookCover={banner.imageLinks !== undefined ?
                  banner.imageLinks.thumbnail : unknownBookCoverAddress
                }
              />
            ))}
          </SwipeableViews>
        ...
        <HomeSection sectionLabel="Currently Reading" linkLabel="All">
          <img
            ...
            width="88px"
            height="130px"
            src={currentlyReadingBook?.imageLinks.thumbnail !== undefined ?
              currentlyReadingBook.imageLinks.thumbnail : unknownBookCoverAddress}
            alt="Book cover"
          />

          <div className={styles.currentlyReading}>
            <p className={styles.title}>
              {currentlyReadingBook?.title !== undefined ?
                currentlyReadingBook.title : "Unknown title"}
            </p>

            <p className={styles.author}>
              by {currentlyReadingBook?.authors[0] !== undefined ?
                currentlyReadingBook?.authors[0] : "Unknown author"}
            </p>
            ...
            <div className={styles.chapterInfo}>
              ...
              <p className={styles.chapterInfoText}>
            ...
            <svg className={styles.oval2} width="116" height="116" viewBox="0 0 116 116" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="58" cy="58" r="58" fill="#D6D1B1" fill-opacity="0.09" />
            </svg>
